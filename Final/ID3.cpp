...
void trainModel(vector<vector<double>> &trainData, vector<int> &trainLabels)
{
	// Your training logic here
}

vector<int> predict(const vector<vector<double>> &testData)
{
	// Your prediction logic here
	return vector<int>(); // Modify the return type accordingly
}

// Function to compute precision, recall, and F1 score
tuple<double, double, double> evaluate(const vector<int> &trueLabels, const vector<int> &predictedLabels)
{
	...
	double precision = (truePositive + falsePositive > 0) ? static_cast<double>(truePositive) / (truePositive + falsePositive) : 0.0;
	double recall = (truePositive + falseNegative > 0) ? static_cast<double>(truePositive) / (truePositive + falseNegative) : 0.0;
	double f1 = (precision + recall > 0) ? (2 * precision * recall) / (precision + recall) : 0.0;

	return make_tuple(precision, recall, f1);
}

// 10-fold cross-validation
void kFoldCrossValidation(const vector<vector<double>> &features, const vector<int> &labels, int k = 10)
{
	size_t dataSize = features.size();
	size_t foldSize = dataSize / k;

	for (int fold = 0; fold < k; ++fold)
	{
		// Split data into training and testing sets
		vector<vector<double>> trainingFeatures, testingFeatures;
		...
		for (size_t i = 0; i < dataSize; ++i)
		{
			...
			{
				testingFeatures.push_back(features[i]);
		...
		vector<int> predictions = predict(testingFeatures);

		// Evaluate the model
		auto [precision, recall, f1] = evaluate(testingLabels, predictions);

		// Print or store the evaluation metrics
		cout << "Fold " << fold + 1 << " - Precision: " << precision << ", Recall: " << recall << ", F1 Score: " << f1 << endl;
	}
}
...
int main(int argc, char *argv[])
{
	...
	if (exptno[0] == '1')
	{
		...
		cout << "Threshold %" << setw(16) << "Test accuracy" << setw(16) << "No. of nodes" << endl
			 << endl;
	...
	else if (exptno[0] == '2')
	{
		...
		cout << "Noise %" << setw(16) << "Test accuracy" << setw(16) << "No. of nodes" << endl
			 << endl;
	...
	else if (exptno[0] == '3')
	{
		...
		cout << "Test accuracy" << setw(16) << "No. of nodes" << endl
			 << endl;
	...
	else if (exptno[0] == '4')
	{
		...
		cout << "No. of trees" << setw(16) << "   Training accuracy" << setw(16) << "Test accuracy" << endl
			 << endl;
